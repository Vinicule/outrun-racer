...
class Opponent {
  ...
  create() {
    ...
    this.sprite.sheetPosX = this.carColor !== 'random'
      ? this.carColor
      : Math.floor(Math.random() * 7.99);
    this.sprite.sheetPosY = 0;
  ...
  * carSpeedCorrection() {
  ...
  update(road, director, player, oppArr) {
    if (director.running) {
      // crash correctorion method to fix the car's movement when colliding with a different object 
