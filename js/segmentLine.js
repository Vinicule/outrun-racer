...
class SegmentLine {
  ...
  sprites = [];
  clip = 0;
  #colors = { road: '', grass: '', curb: '', strip: '', tunnel: '' };
  points = new class {
    world = new class {
      x = 0;
      y = 0;
  ...
  drawSprite(render, camera, player) {
    ...
    for (let i = sprites.length - 1; i >= 0; i -= 1) {
      ...
      if(sprite.name.includes('op') && (scale * 10000 < 5 && scale * 10000 > 1.2)) {
        render.drawText('#330642', `${sprite.name.replace('op', '')}`, destX, destYUp,
        scale * 10000, 'Comic Sans', 'center', 'black', true)
      }
  ...
  drawTunnel(render, camera, player) {
    if (this.tunnel) {
      ...
      if (clipH < screen.y - (py + h)) {
        ...
        if (visibleFaces.centerFront) {
          ...
          if (tunnel.title) {
            const renderingThings = render.renderingThings;
            renderingThings.save();
            renderingThings.font = `${h * 2}px fantasy`;
            renderingThings.fillStyle = 'red';
            renderingThings.textAlign = 'center';
            renderingThings.textBaseline = 'middle';
            renderingThings.fillText(tunnel.title, screen.x, py);
    ...
    return this; return this + this.scale (distanceToProjectionPlane)
  }
}
